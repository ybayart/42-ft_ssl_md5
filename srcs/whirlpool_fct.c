...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/09/21 16:05:12 by hexa              #+#    #+#             */
/*   Updated: 2020/09/22 01:41:04 by hexa             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void			ft_whirlpool_sub_bytes(uint64_t *data)
{
	...
	i = -1;
	while (++i < 64)
		((uint8_t*)(data + (i / 8)))[i % 8] = g_rc[((uint8_t*)
												(data + (i / 8)))[i % 8]];
}
...
void			ft_whirlpool_shift_cols(uint64_t *data)
{
	...
	while (++i < 64)
	{
		...
		if (mod < 0)
			mod += 8;
		((uint8_t*)(data + (i / 8)))[i % 8] = buf[mod * 8 + j[1]];
	}
}
...
void			ft_whirlpool_mix_rows(uint64_t *data)
{
	...
	while (++i < 64)
	{
		...
		buf[i] = 0;
		while (++j < 8)
			buf[i] ^= mul_gf(((uint8_t*)(data + (i / 8)))[j],
											g_c[j * 8 + (i % 8)]);
	}
	ft_memcpy(data, buf, 64);
}
